...
class G08AEvaluator:
    ...
    def parse_teams_from_filename(self, filename):
        ...
        if base.startswith('hybrid_'):
            # It's a hybrid experiment, need to read team names from inside JSON
            try:
                ...
                # Infer team assignment:
                all_players = list(biddings.keys())
                # Assume half are team1, half are team2 (because hybrid uses balanced teams)
                n = len(all_players)
                team1_players = set(all_players[:n//2])
                team2_players = set(all_players[n//2:])
                return list(team1_players), list(team2_players)
            except Exception as e:
                logging.error(f"Failed to parse hybrid file {filename}: {e}")
