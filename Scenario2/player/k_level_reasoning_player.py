...
from ollama_client import call_ollama  # New import for ollama client
...
class KLevelReasoningPlayer(AgentPlayer):
    ...
    def start_round(self, round):
        ...
        self.message.append({"role": "system", "content": self.INQUIRY_COT.format(name=self.name, round=round, prediction=prediction_str, hp=self.hp)})
    ...
    def predict(self, round):
        ...
        for k in range(self.k_level):
            ...
            prediction_str = ", ".join([f"{player} might choose {prediction[player]}" for player in prediction]) + ". "
            self_message.append({"role": "system", "content": self.INQUIRY_COT.format(name=self.name, round=k_round, prediction=prediction_str, hp=self_hp)})
            ...
            bidding_numbers = " + ".join(str(prediction[player]) for player in prediction)
            for player in history_biddings:
            ...
            diff_details = ", ".join(f"{player}: |{prediction[player]} - {Target}| = {round_number(abs(prediction[player] - Target))}" for player in prediction)
