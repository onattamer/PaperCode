...
from ollama_client import call_ollama  # New: use our custom function for Qwen calls
...
class KLevelReasoningPlayer(AgentPlayer):
    ...
    def start_round(self, round):
        ...
        self.message += [{"role": "system", "content": self.INQUIRY_COT.format(name=self.name, round=round, prediction=prediction_str, hp=self.hp)}]
    ...
    def notice_round_result(self, round, bidding_info, round_target, win, bidding_details, history_biddings):
        ...
        self.history_biddings = history_biddings  # e.g., {"Alex": [1,2,3]}
    ...
    def predict(self, round):
        def self_act(message):
            ...
            while status != 1:
                try:
                    ...
                    print(e)
                    time.sleep(15)
            return self.parse_result(response_content)
        ...
        for k in range(self.k_level):
            ...
            Average = sum(prediction[player] for player in prediction) / len(prediction)
            Target = round_number(Average * 0.8, 2)

            Tie_status = len(prediction) >= 2 and len(set(prediction.values())) == 1
            if Tie_status:
                winners = []
            else:
            ...
            bidding_numbers = " + ".join([f"{prediction[player]}" for player in prediction])
            for player in history_biddings:
                history_biddings[player].append(prediction[player])
            bidding_details = ", ".join([f"{player} chose {prediction[player]}" for player in prediction])
            diff_details = ", ".join([
                f"{player}: |{prediction[player]} - {Target}| = {round_number(abs(prediction[player] - Target))}"
                for player in prediction
            ])
            player_details = ", ".join([f"NAME:{player}\tHEALTH POINT:{player_hp[player]}" for player in prediction])

            if Tie_status:
                bidding_info = (f"Thank you all for participating in Round {k_round}. In this round, {bidding_details}.\n"
                                "All players chose the same number, so all players lose 1 point. "
                                f"After the deduction, player information is: {player_details}.")
            else:
                bidding_info = (f"Thank you all for participating in Round {k_round}. In this round, {bidding_details}.\n"
                                f"The average is ({bidding_numbers}) / {len(prediction)} = {Average}.\n"
                                f"The average {Average} multiplied by 0.8 equals {Target}.\n"
                                f"{diff_details}\n"
                                f"{winners}'s choice of {win_bid} is closest to {Target}. Round winner: {winner_str}. "
                                f"All other players lose 1 point. After the deduction, player information is: {player_details}.")
